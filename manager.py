...
class Run():
    ...
    def runlog(self):
        ...
        print 'run_time_sum : %.8f sec' % self.run_time_sum

    def thread1(self):
        for thread1_test_case in self.test_cases[: 2]:
            time_s = time.time()
            ...
            print "thread1_test_case : %s" % thread1_test_case.result

    def thread2(self):
        for thread2_test_case in self.test_cases[2: ]:
            time_s = time.time()
            thread2_test_case.result = os.popen('python  %s' % thread2_test_case.file).read()
            time_e = time.time()
            thread2_test_case.run_time = time_e-time_s
            print "thread2_test_case : %s" % thread2_test_case.result
    ...
    def run_test_case(self):
        ...
        thread2.join()
        time_end = time.time()
        self.run_time_sum = time_end-time_start
        self.runlog()
